...
.soutien{width:400px; text-aligne:center; display:inline-block;}
</style>
	<?php 
	$titre="cooperative";
	include("../header.php") ?>
...
$membres = $db->query('SELECT * FROM membres WHERE statut="travailleur" OR statut="soutienCA";')->fetchAll();
?>
	<div id="cache"></div>
<?php
...
foreach($membres as $donnee) {
	...
	echo "</h2>";
	if(	$donnee ['photo']<>"membres/"){echo "<img src='../admin/".$donnee ['photo']."' width='150'/>"; }; if($_SESSION['admin']){echo "<br><input type='hidden' name='MAX_FILE_SIZE' value='12000000' />	<input type='file' name='photo'/>";}
	echo "<h2>";
	if($_SESSION['admin']){echo "<select name='fonction'>
							...
							<option value='Autre' "; if(html_entity_decode(stripslashes($donnee ['fonction']))=='Autre'){echo"selected";} echo ">Autre</option>
						</select>";
						echo " auxiliaire<input type='checkbox' name='aux' value='TRUE' "; if($donnee ['aux']=='TRUE'){echo "checked";} echo ">";
	echo "</h2>";
	}
	else{ if($donnee ['aux']=='TRUE'){echo " (membre auxiliaire)";}else{echo "<h2>".html_entity_decode(stripslashes($donnee ['fonction']));}; echo "</h2>";}
...
?>
		...
		<article>
			<h2>Membres travailleuses - fondatrices</h2>
<?php
$membres_t = $db->query('SELECT * FROM membres WHERE statut="travailleur" AND aux!="TRUE" ORDER BY nom;')->fetchAll();
...
foreach($membres_s as $donnee) {
	echo "<span class='soutien'>";
	if($_SESSION['admin']){
		echo "<form method='post' action='../admin/modifier/membre.php' style='display:inline;'>
					<input type='hidden' name='action' value='modifier' />
					<input type='hidden' name='id' value='".$donnee ['id']."' />
					<input type='text' name='prenom' value='".$donnee ['prenom']."' size=10/>
					<input type='text' name='nom' value='".$donnee ['nom']."' size=10/>
					<input type='text' name='ville' value='".$donnee ['bio']."' size=10/>
					<input type = 'submit' name='modifier' value='Modifier'> <input type = 'submit' name='supprimer' value='x'></form>";
	}
	else{
		echo $donnee ['prenom']." ".$donnee ['nom'].", ".$donnee['bio'];
	}
	echo "</span>";
}
?>
