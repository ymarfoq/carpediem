...
?>
		...
		<table>
			<tr>
<?php
	$photos = $db->query('SELECT * FROM photos WHERE id=3 OR id=4 OR id=5;')->fetchAll();
foreach($photos as $donnee){
	echo "<td>";
	if($donnee['photo']<>"-"){
		echo "<img src='../admin/".$donnee['photo']."' width=200>";
		echo "<h2>".$donnee['legende']."</h2>";
	}
	if($_SESSION['admin']){
...
?>
	...
	<td width=300>
	<?php
	$photos = $db->query('SELECT * FROM photos WHERE id=1 OR id=2;')->fetchAll();
foreach($photos as $donnee){
