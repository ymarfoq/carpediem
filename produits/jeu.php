...
#dice .frontin  {transform: translateZ(75px); -webkit-transform: translateZ(75px);}
#dice .back {transform: rotateX(-180deg) translateZ(100px); -webkit-transform: rotateX(-180deg) translateZ(100px);}
#dice .backin {transform: rotateX(-180deg) translateZ(75px); -webkit-transform: rotateX(-180deg) translateZ(75px);}
#dice .right {transform: rotateY(90deg) translateZ(100px); -webkit-transform: rotateY(90deg) translateZ(100px);}
#dice .rightin {transform: rotateY(90deg) translateZ(75px); -webkit-transform: rotateY(90deg) translateZ(75px);}
#dice .left {transform: rotateY(-90deg) translateZ(100px); -webkit-transform: rotateY(-90deg) translateZ(100px);}
#dice .leftin {transform: rotateY(-90deg) translateZ(75px); -webkit-transform: rotateY(-90deg) translateZ(75px);}
#dice .top {transform: rotateX(90deg) translateZ(100px); -webkit-transform: rotateX(90deg) translateZ(100px);}
#dice .topin {transform: rotateX(90deg) translateZ(75px); -webkit-transform: rotateX(90deg) translateZ(75px);}
#dice .bottom {transform: rotateX(-90deg) translateZ(100px); -webkit-transform: rotateX(-90deg) translateZ(100px);}
#dice .bottomin {transform: rotateX(-90deg) translateZ(75px); -webkit-transform: rotateX(-90deg) translateZ(75px);}
...
<?php 
	...
	foreach($cdc_jeux as $donnee){
		echo "<div class='side out ".$faces[$face]."'><div><h3>".html_entity_decode(stripslashes($donnee['titre']))."</h3><img src='../admin/".$donnee['photo']."' width=150 height=130></div></div>";
		echo "<div class='side ".$faces[$face]."in'></div>";
	$face++;
	}
	
?>
      <div class="side out left"><div><a href='./'>Plus</a></div></div>
      <div class='side leftin'></div>
    </div>
  </div>
</div>
<script>
